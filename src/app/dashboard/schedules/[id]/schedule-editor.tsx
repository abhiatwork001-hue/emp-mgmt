...
import {
    ChevronLeft, ChevronRight, Save, User, ArrowLeft, Loader2, Copy, Trash2, X, Plus, Filter, Tag, Check, XCircle, Clock, Calendar,
...
export function ScheduleEditor({ initialSchedule, userId }: { initialSchedule: any, userId: string }) {
    ...
    // Derived State
    const isEditMode = schedule.status === 'draft' || schedule.status === 'rejected';
    ...
    const handleSaveShift = (shiftData: any) => {
        ...
        saveChanges(newWeekDays);
        setEditingShift(null); // Reset after save
    };

    const saveChanges = async (newDays: any[]) => {
        setIsSaving(true);
        try {
            await updateSchedule(schedule._id, { days: newDays });
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className="hidden md:block">
                        <div className="flex items-center gap-2">
                            ...
                            {!isEditMode && <Badge variant="secondary" className="gap-1"><Lock className="h-3 w-3" /> View Only</Badge>}

                            {/* Saving Indicator */}
                            {isSaving ? (
                                <Badge variant="outline" className="gap-1 text-muted-foreground border-transparent">
                                    <Loader2 className="h-3 w-3 animate-spin" /> Saving...
                                </Badge>
                            ) : lastSaved ? (
                                <Badge variant="outline" className="gap-1 text-green-600 border-transparent">
                ...
                <div className="flex items-center gap-2 flex-wrap">
                    {(schedule.status === 'draft' || schedule.status === 'rejected') && (
                        <>
                            ...
                            {!weekDays.every(d => d.isHoliday || d.shifts.length > 0) && (
                                <p className="text-xs text-muted-foreground w-full text-right">Compute all days to enable</p>
                            )}
                        </>
                    )}
                    ...
                    <div className="flex items-center space-x-2 mr-2">
                        ...
                        />
                        <Label htmlFor="hide-unscheduled" className="text-xs">Scheduled Only</Label>
                    </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left">
                        <thead className="bg-muted/50 text-muted-foreground">
                            <tr>
                                <th className="p-4 min-w-[200px] font-medium sticky left-0 bg-background/95 backdrop-blur z-10 border-r border-border">Employee</th>
                                {weekDays.map((day: any, i: number) => {
                                    const isCurrentDay = isToday(new Date(day.date));
                                    return (
                                        <th key={i} className={`p-4 min-w-[150px] font-medium border-r border-border last:border-r-0 text-center group relative transition-colors ${day.isHoliday ? 'bg-muted/50' : isCurrentDay ? 'bg-blue-500/10' : 'hover:bg-muted/80'}`}>
                                            <div className="flex flex-col items-center">
                                                <span className={`uppercase text-xs font-bold ${isCurrentDay ? 'text-blue-600' : ''}`}>{new Date(day.date).toLocaleDateString('en-US', { weekday: 'short' })}</span>
                                                <div className={`text-xs font-normal w-6 h-6 flex items-center justify-center rounded-full mt-1 ${isCurrentDay ? 'bg-blue-600 text-white' : ''}`}>
                                                ...
                                                <Popover>
                                                    ...
                                                    <PopoverContent className="w-60">
                                                        <div className="space-y-4">
                                                            <h4 className="font-medium leading-none">Day Settings</h4>
                                                            <div className="flex items-center space-x-2">
                                                                <Switch
                                                                    ...
                                                                    onCheckedChange={(checked) => {
                                                                        ...
                                                                        saveChanges(updatedDays);
                                                                    }}
                                                                />
                                                                <Label htmlFor={`closed-${i}`}>Store Closed</Label>
                                                            </div>
                                                            {day.isHoliday && (
                                    ...
                                    <tr key={emp._id} className="hover:bg-muted/30">
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="flex flex-col gap-0.5 mt-1">
                                                        <div className={`text-[10px] items-center gap-1 flex ${isUnderHours ? "text-amber-500 font-bold" : "text-emerald-500"}`}>
                                                            <Clock className="h-3 w-3" />
                                                            {totalHours.toFixed(1)} / {contractHours}h
                                                        </div>
                                                        {isUnderDays && (
                                                            <div className="text-[10px] text-amber-500 font-bold flex items-center gap-1">
                                                                <Calendar className="h-3 w-3" />
                                                                {totalShifts} / {contractDays} Days
                                                            </div>
                                                        )}
                                                    </div>
                                                </div>
                                        ...
                                        {weekDays.map((day: any, i: number) => {
                                            ...
                                            return (
                                                <td
                                                    key={i}
                                                    className={`p-2 border-r border-border last:border-r-0 align-top transition-colors ${day.isHoliday ? 'bg-muted/30' : isCurrentDay ? 'bg-blue-500/5' : (isEditMode ? 'cursor-pointer hover:bg-muted/50' : '')}`}
                                                        ...
                                                        <div className="space-y-1">
                                                            {empShifts.map((shift: any, idx: number) => {
                                                                ...
                                                                if (isDayOff) {
                                                                    return (
                                                                        ...
                                                                        >
                                                                            ...
                                                                            <span className="font-bold uppercase text-[10px]">Day Off</span>
                                                                        </div>
                                                                    );
                                                                }
                                                                ...
                                                                const shiftStyle = shift.color ? {
                                                                    backgroundColor: `${shift.color}15`, // 10% opacity
                                                                    borderColor: `${shift.color}40`, // 25% opacity
                                                                    color: shift.color
                                                                } : {};
                                                                ...
                                                                return (
                                                                    <div
                                                                        ...
                                                                        draggable={isEditMode}
                                                                        onDragStart={isEditMode ? (e) => handleDragStart(e, shift) : undefined}
                                                                        className={`p-2 rounded text-xs border transition-colors group relative ${!shift.color ? 'bg-blue-500/10 text-blue-500 border-blue-500/20' : ''} ${isEditMode ? 'cursor-grab active:cursor-grabbing hover:opacity-80' : ''} ${absence ? 'border-red-500/50 bg-red-500/10' : ''}`}
            ...
            <AlertDialog open={copyDialogOpen} onOpenChange={setCopyDialogOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This will overwrite all shifts in the current week with shifts from the previous week. This action cannot be undone.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel>Cancel</AlertDialogCancel>
                        <AlertDialogAction onClick={handleCopyLastWeekConfirm}>Confirm</AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
            </AlertDialog>
            ...
            <AlertDialog open={deleteShiftAlertOpen} onOpenChange={setDeleteShiftAlertOpen}>
                <AlertDialogContent>
                    ...
                    <AlertDialogFooter>
                        ...
                        <AlertDialogAction className="bg-destructive hover:bg-destructive/90" onClick={executeDeleteShift}>Delete</AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
            </AlertDialog>
            ...
            <AlertDialog open={statusConfirmOpen} onOpenChange={setStatusConfirmOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Are you sure you want to change the status to <span className="font-semibold capitalize">{pendingStatus}</span>?
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
            ...
            <Dialog open={rejectDialogOpen} onOpenChange={setRejectDialogOpen}>
                <DialogContent>
                    ...
                    <div className="py-4">
                        ...
                        <Textarea
                            id="rejection-reason"
                            placeholder="Please explain why the schedule is being rejected..."
                    ...
                    <DialogFooter>
                        ...
                        <Button variant="destructive" onClick={handleRejectConfirm} disabled={!rejectionReason}>Reject</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
